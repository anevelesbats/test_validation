...
#define COMPARE_FILE "txt/compare.txt"
#define VHD_FILE "txt/generate_tb.txt"
...
void generate_tb(){
    ...
    const char *filename = VHD_FILE;
    remove(filename);
    FILE *file = fopen(filename, "w");
    
    const char *filename_result = RESULT_FILE;
    remove(filename_result);
    FILE *file_result = fopen(filename_result, "w");

    for(int i = 0; i < 100; i++){
        int a = rand_A();
        int b = rand_B();
        int result = pgcd_result(a, b, file_result);
        fprintf(file, "idata_a <= STD_LOGIC_VECTOR(TO_UNSIGNED(%d, 32));\n", a);
        fprintf(file, "idata_b <= STD_LOGIC_VECTOR(TO_UNSIGNED(%d, 32));\n", b);
        fprintf(file, "idata_en <= '1';\n");
        fprintf(file, "wait for 10 ns;\n"); 
        fprintf(file, "while odata_en = '0' loop\n");
        fprintf(file, "    idata_en <= '0';\n"); 
        fprintf(file, "    wait for 10 ns;\n");
        fprintf(file, "end loop;\n");
        fprintf(file, "ASSERT UNSIGNED(odata) = TO_UNSIGNED(%d, 32) SEVERITY FAILURE;\n\n", result);
        fprintf(file, "wait for 20 ns;\n"); 
